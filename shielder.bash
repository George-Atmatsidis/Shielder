...
read maxtries
sudo sed -i 's/\(#MaxAuthTries\).*/\MaxAuthTries '$maxtries'/' /etc/ssh/sshd_config
##################################
...
read maxstartups
sudo sed -i 's/\(#MaxStartups\).*/\MaxStartups '$maxstartups'/' /etc/ssh/sshd_config
##################################
...
sudo sed -i "s/#UsePrivilegeSeparation sandbox/UsePrivilegeSeparation sandbox/" /etc/ssh/sshd_config
##################################
